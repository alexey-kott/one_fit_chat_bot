...
from playhouse.shortcuts import model_to_dict, \
    dict_to_model  # для сериализации peewee-объектов во время логирования ошибок

# ______ модули приложения
import bot.string_resources as s  # все строки хранятся здесь
import bot.string_resources as string

from bot.check import check_age, check_email, check_height, \
    check_weight  # различные проверки: правильно ли юзер ввёл рост/вес/etc
from bot.config import WEBHOOK_HOST, API_TOKEN
from bot.functions import send_mail, init_routing, log_routing_error

# импорт моделей
from .bot_models import User, Routing, Trainer, Error, Photo, Message, Schedule


aiobot = Bot(token=API_TOKEN)
...
async def handle(request):
    if request.match_info.get('token') == aiobot.token:
        request_body_dict = await request.json()
        update = types.Update.de_json(request_body_dict)
        aiobot.process_new_updates([update])
        return web.Response()
    else:
...
def send_message_delay(chat_id, m, state=None, delay=0, reply_markup=None, disable_notification=None,
                       parse_mode='Markdown'):
    u = User.get(user_id=chat_id)
    aiobot.send_chat_action(chat_id, 'typing')
    if state != None:
        u.state = state
    u.save()
    aiobot.send_message(chat_id, m, reply_markup=reply_markup, parse_mode=parse_mode,
...
def send_photo_delay(chat_id, p, state=None, delay=0, disable_notification=None):
    ...
    if state != None:
        u.state = state
    u.save()
    aiobot.send_photo(chat_id, p, disable_notification=disable_notification)
...
def cancel(u, c=None, m=None):
    u.state = s.canceled
    u.save()
    aiobot.send_message(u.user_id, s.canceled_course)
...
def three_days_importance(u, c):
    ...
    aiobot.send_chat_action(cid(c), 'upload_video')
    aiobot.send_video(cid(c), video, reply_markup=keyboard)
...
def confirm_name(u, c):
    ...
    keyboard.add(agree_btn)
    aiobot.send_message(cid(c), s.confirm_name.format(u.first_name), reply_markup=keyboard)


def confirm_last_name(u, m=None,
                      c=None):  # получает имя (текстом) или подтверждение того, что мы правильно записали его имя
    if m != None:
        chat_id = uid(m)
        u.first_name = m.text
        keyboard = types.InlineKeyboardMarkup()
        aiobot.edit_message_reply_markup(uid(m), message_id=int(m.message_id) - 1, reply_markup=keyboard)
    else:
        chat_id = cid(c)
    ...
    if u.last_name != None:
        ...
        keyboard.add(agree_btn)
        aiobot.send_message(chat_id, s.confirm_last_name.format(u.last_name), reply_markup=keyboard)
        return
    aiobot.send_message(chat_id, s.type_last_name.format(u.last_name), reply_markup=keyboard)
...
def select_sex(u, m=None, c=None):
    ...
    aiobot.send_message(chat_id, s.male_or_female, reply_markup=keyboard)
...
def type_age(u, c=None):
    ...
    u.state = s.age
    u.save()
    aiobot.send_message(cid(c), s.type_age)


def incorrect_age(u, m):
    aiobot.send_message(uid(m), s.incorrect_age)


def type_email(u, m):
    if not check_age(m.text):
        incorrect_age(u, m)
        return False
    u.age = check_age(m.text)
    u.state = s.email
    u.save()
    aiobot.send_message(uid(m), s.type_email)


def incorrect_email(m):
    aiobot.send_message(uid(m), s.incorrect_email)


def video_intro(u, m):
    if not check_email(m.text):
        incorrect_email(m)
        return False
    u.email = check_email(m.text)
    u.state = s.video_intro
    u.save()

    aiobot.send_message(uid(m), s.what_to_do)
    video = open('videos/first_three_days.mp4', 'rb')
    keyboard = types.InlineKeyboardMarkup()
    agree_btn = types.InlineKeyboardButton(text=s.looked_btn, callback_data=s.agree)
    keyboard.add(agree_btn)
    aiobot.send_chat_action(uid(m), 'upload_video')
    aiobot.send_video(uid(m), video, reply_markup=keyboard)
...
def are_we_continue(u, c):
    ...
    keyboard.add(agree_btn)
    aiobot.send_message(cid(c), s.are_we_continue, reply_markup=keyboard)
...
def present_trainer(u, c):
    ...
    aiobot.send_photo(cid(c), photo, s.your_trainer.format(t.first_name, t.last_name))

    keyboard = types.InlineKeyboardMarkup()
    agree_btn = types.InlineKeyboardButton(text=s.agree_btn, callback_data=s.agree)
    disagree_btn = types.InlineKeyboardButton(text=s.disagree_btn, callback_data=s.disagree)
    keyboard.add(agree_btn, disagree_btn)
    send_message_delay(cid(c), s.are_you_ready, state=s.ready, reply_markup=keyboard, disable_notification=True,
                       delay=5)  # "Вы готовы?"
...
def remind_1(u, c):
    u.state = s.stop
    u.save()
    aiobot.send_message(u.user_id, s.waiting_from_you)
...
def city(u, c):
    u.state = s.city
    u.save()
    aiobot.send_message(cid(c), s.type_city)


def job(u, m):
    u.city = m.text
    u.save()
    aiobot.send_chat_action(uid(m), 'typing')
...
def height(u, m):
    ...
    u.state = s.height
    u.save()
    aiobot.send_message(uid(m), s.type_height)


def incorrect_height(chat_id):
    aiobot.send_message(chat_id, s.incorrect_height)


def weight(u, m):
    if not check_height(m.text):
        incorrect_height(uid(m))
        return False
    u.height = check_height(m.text)
    u.state = s.weight
    u.save()
    aiobot.send_message(uid(m), s.type_weight)


def incorrect_weight(chat_id):
    aiobot.send_message(chat_id, s.incorrect_weight)


def target_weight(u, m):
    if not check_weight(m.text):
        incorrect_weight(uid(m))
        return False
    u.state = s.target_weight
    u.weight = m.text
    u.save()
    aiobot.send_message(uid(m), s.type_target_weight)


def any_methodologies(u, m):
    u.target_weight = m.text
    u.save()
    aiobot.send_message(uid(m), s.thanks_for_answers)
    keyboard = types.InlineKeyboardMarkup()
    agree_btn = types.InlineKeyboardButton(text=s.agree_btn, callback_data=s.agree)
    disagree_btn = types.InlineKeyboardButton(text=s.disagree_btn, callback_data=s.disagree)
    keyboard.add(agree_btn, disagree_btn)
...
def what_methodologies(u, c):
    ...
    u.save()
    aiobot.send_message(cid(c), s.what_methodologies)
...
def thanks_for_efforts(user_id):
    aiobot.send_message(user_id, s.thanks_for_efforts)
...
def day_2(user_id):
    ...
    aiobot.send_chat_action(user_id, 'typing')
...
def last_analyzes(u, m=None, c=None):
    if m != None:
        ...
        try:
            ...
            pass
    u.save()
    aiobot.send_chat_action(u.user_id, 'typing')
...
def day_2_end(user_id):
    ...
    aiobot.send_message(user_id, s.day_2_end)
...
def day_3(user_id):
    ...
    u.day = 3
    u.save()
    aiobot.send_message(user_id, s.greeting_3.format(u.first_name))
...
def relatives_attitude(u, m=None, c=None):
    ...
    aiobot.send_chat_action(u.user_id, 'typing')
    send_message_delay(u.user_id, s.your_relatives_attitude, state=s.relatives_attitude, reply_markup=keyboard,
                       delay=15)
...
def final_reminder(u, m):
    ...
    u.state = s.final
    u.save()
    aiobot.send_message(uid(m), s.fill_table)
    aiobot.send_chat_action(u.user_id, 'typing')
...
def bye_day_3(user_id):
    ...
    u.day = 0
    u.save()
    aiobot.send_message(user_id, s.bye_day_3)


# _____________ END ACTIONS

@aiobot.message_handler(commands=['ping'])
def ping(m):
    aiobot.send_message(uid(m), "I'm alive")


@aiobot.message_handler(commands=['init'])
def init(m):
...
@aiobot.message_handler(commands=['start'])
def start(m):
    # print(u, m)
    u = User.cog(user_id=uid(m), username=m.from_user.username, first_name=m.from_user.first_name,
    ...
    aiobot.send_message(uid(m), s.greeting, reply_markup=keyboard, parse_mode="Markdown")


@aiobot.callback_query_handler(func=lambda call: True)
def clbck(c):
    chat_id = cid(c)
    ...
    try:
        ...
        keyboard = types.InlineKeyboardMarkup()
        aiobot.edit_message_reply_markup(chat_id=cid(c), message_id=c.message.message_id, reply_markup=keyboard)
        aiobot.answer_callback_query(callback_query_id=c.id, show_alert=True)
...
@aiobot.message_handler(content_types=['text'])
def action(m):
    chat_id = sid(m)
...
def save_photo(message):  # системная функция, не вызывает отправку сообщения в ТГ
    ...
    file_info = aiobot.get_file(fileID)
    downloaded_file = aiobot.download_file(file_info.file_path)
...
@aiobot.message_handler(content_types=['photo'])
def photo(m):
    ...
    def __call__(self):
        while True:
            ...
            users = User.select().where(
                ...
                (User.state != s.stop))
            for user in users:
                aiobot.send_message(user.user_id, s.one_hour_reminder)
            sleep(1)
