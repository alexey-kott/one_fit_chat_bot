INSERT INTO routing VALUES 
	('pause','text','idle'),
	('default','agree','three_days_importance'),
	('three_days_importance','agree','confirm_name'),
	('default','disagree','cancel'),
	('lets_confirm_name','agree','confirm_last_name'),
	('lets_confirm_name','text','confirm_last_name'),
	('lets_confirm_last_name','agree','select_sex'),
	('lets_confirm_last_name','text','select_sex'),
	('sex','male','get_acquainted'),
	('sex','female','get_acquainted'),
	('acquaintance','agree','type_age'),
	('age','text','type_email'),
	('email','text','video_intro'),
	('video_intro','agree', 'are_we_continue'),
	('after_intro','agree', 'present_trainer'),
	('trainer','agree', 'are_you_ready'),
	('ready','agree', 'remind_1'),
	('ready','disagree', 'not_ready'),
	('remind_1','agree', 'city'),
	('city','text', 'job'),
	('job','text', 'height'),
	('height','text', 'weight'),
	('weight','text', 'target_weight'),
	('target_weight','text', 'any_methodologies'),
	('any_methodologies','agree', 'what_methodologies'),
	('any_methodologies','disagree', 'waiting_from_you'),
	('what_methodologies','text', 'most_difficult'),
	('most_difficult','text', 'was_result'),
	('was_result','text', 'why_fat_again'),
	('why_fat_again','text', 'waiting_from_you'),
	('measurements','agree', 'idle'),
	('day_2','text', 'when_start_fat'),
	('day_2','photo', 'when_start_fat'),
	('start_fat','text', 'why_fat_now'),
	('why_fat','text', 'hormonals'),
	('hormonals','text', 'last_analyzes'),
	('hormonals','disagree', 'last_analyzes'),
	('last_analyzes','text', 'not_eat'),
	('not_eat','text', 'allergy'),
	('allergy','text', 'last_2_day_answer'),
	('day_3','text', 'fats_in_family'),
	('day_3','photo', 'fats_in_family'),
	('fats_in_family','text', 'fat_children'),
	('fats_in_family','disagree', 'fat_children'),
	('fat_children','disagree', 'relatives_attitude'),
	('fat_children','text', 'relatives_attitude'),
	('relatives_attitude','text', 'amount_of_walking'),
	('relatives_attitude','support', 'amount_of_walking'),
	('relatives_attitude','dissuade', 'amount_of_walking'),
	('amount_of_walking','text', 'any_injuries'),
	('amount_of_walking','many', 'any_injuries'),
	('amount_of_walking','few', 'any_injuries'),
	('amount_of_walking','middling', 'any_injuries'),
	('any_injuries','text', 'motivation'),
	('any_injuries','disagree', 'motivation'),
	('motivation','text', 'final_reminder'),
	('final','text','last_3_day_answer'),
	('day_3_end','text','idle');
